<script lang="ts">
    ...
    let redPlayers = $derived(Game.players.filter(p => p.team === 2).length);
    let bluePlayers = $derived(Game.players.filter(p => p.team === 3).length);
</script>

<div class="w-full h-full flex flex-col items-center">
    <h2 class="w-full text-center text-5xl pb-2">Players</h2>
    <table class="overflow-y-auto block" style="width: min(100%, 900px)">
        <thead class="sticky top-0 bg-background">
            <tr class="*:pr-1.5">
                <th class="min-w-10"></th> 
                <th class="w-full text-left font-semibold">
                    <span style="color: {nameColors[3]}">{bluePlayers}</span>
                    -
                    <span style="color: {nameColors[2]}">{redPlayers}</span>
                </th>
                {#if columns.class}
                    <th class="min-w-8"></th>
                {/if}
